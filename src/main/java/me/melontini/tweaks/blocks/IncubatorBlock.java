package me.melontini.tweaks.blocks;
...
public class IncubatorBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, BlockRegistry.INCUBATOR_BLOCK_ENTITY, IncubatorBlockEntity::tick);
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        tooltip.add(TweaksTexts.INCUBATOR_0);
        if (Tweaks.CONFIG.unknown)
            tooltip.add(TweaksTexts.INCUBATOR_1);
    }
