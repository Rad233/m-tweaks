    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (hitResult.getType() == HitResult.Type.MISS) {
            ...
            if (hitResult.getType() == HitResult.Type.BLOCK) {
                ChestBoatEntity chestBoat = new ChestBoatEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z);
                chestBoat.setBoatType(this.type);
                chestBoat.setYaw(user.getYaw());
                if (!world.isSpaceEmpty(chestBoat, chestBoat.getBoundingBox())) {
