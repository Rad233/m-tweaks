...
public class DamageCommand {
    private static final SimpleCommandExceptionType INVULNERABLE_EXCEPTION = new SimpleCommandExceptionType(TextUtil.translatable("m-tweaks.commands.damage.invulnerable"));
    ...
    private static int execute(ServerCommandSource source, Entity target, float amount, DamageSource damageSource) throws CommandSyntaxException {
        if (target.damage(damageSource, amount)) {
            source.sendFeedback(TextUtil.translatable("m-tweaks.commands.damage.success", amount, target.getDisplayName()), true);
            return 1;
        } else {
